...
struct DigitizerState
{
	...
	DigitizerState()																							// default constructor
		:Xaxis(0),Yaxis(0),TipPressure(0),Xtilt(0),Ytilt(0),Eraser(0),Invert(0),BarrelSwitch(0),TipSwitch(0),InRange(0)
	{
		// empty body
	...
	bool operator==(const DigitizerState &right) const															// operator ==
	{
		return Xaxis == right.Xaxis && Yaxis == right.Yaxis && TipPressure == right.TipPressure &&
			Xtilt == right.Xtilt && Ytilt == right.Ytilt && Eraser == right.Eraser && Invert == right.Invert &&
